defmodule ClusterMurmur.Persistence.StochasticEventCommitStore do
  ...
  @claim_keys StochasticScheduleClaim.__struct__() |> Map.keys()
  @claim_key_count length(@claim_keys)
  @claim_lease_seconds 60
  @claim_token_bytes 32

  @type error ::
          :invalid_stochastic_event_commit
  ...
  def commit(_plan, _event, _recorded_at), do: {:error, :invalid_stochastic_event_commit}
  ...
  defp validate(plan, event, recorded_at) do
    with true <- exact_plan?(plan),
         ...
         :ok <- validate_local_date(plan.local_date),
         {:ok, expected_event} <-
      ...
      _failure -> {:error, :invalid_stochastic_event_commit}
    end
  end

  defp validate_claim(%StochasticScheduleClaim{} = claim) do
    with true <- exact_claim?(claim),
         ...
         true <-
           DateTime.compare(
             DateTime.add(claim.started_at, @claim_lease_seconds, :second),
             claim.expires_at
           ) == :eq do
      :ok
    else
  ...
  defp validate_claim(_claim), do: {:error, :invalid_stochastic_event_commit}
  ...
  defp validate_local_date(nil), do: :ok
  defp validate_local_date(%Date{year: year}) when year in 0..9999, do: :ok
  defp validate_local_date(_local_date), do: {:error, :invalid_stochastic_event_commit}
  ...
  defp insert_event(event) do
    case EventStore.insert(event) do
      ...
      {:error, :event_conflict} -> {:error, :stochastic_event_conflict}
      {:error, :invalid_event} -> {:error, :invalid_stochastic_event_commit}
      _failure -> {:error, :storage_unavailable}
    end
  end

  defp advance_schedule(plan, recorded_at) do
    case StochasticScheduleStore.record_execution(
           plan.claim,
           plan.executed_at,
           recorded_at,
           plan.next_run_at,
      ...
      {:error, :invalid_schedule} -> {:error, :invalid_stochastic_event_commit}
      {:error, :storage_unavailable} -> {:error, :storage_unavailable}
      _conflict -> {:error, :stochastic_event_conflict}
    end
  end
