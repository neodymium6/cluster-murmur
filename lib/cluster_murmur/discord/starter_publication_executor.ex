defmodule ClusterMurmur.Discord.StarterPublicationExecutor do
  @moduledoc """
  Executes and closes one exact started starter-message publication.
  ...
  defmodule Outcome do
    ...
    @type t :: %__MODULE__{
            started: ClusterMurmur.Discord.StarterPublicationStarter.Started.t(),
  ...
  def execute(
    ...
    with :ok <- StarterPublicationStarter.validate(started, configuration, cooldowns, settings),
  ...
  def validate(
    ...
    if exact_outcome?(outcome) and
         StarterPublicationStarter.validate(outcome.started, configuration, cooldowns, settings) ==
  ...
  defp publish(started, settings, completed_at, transport, publisher, store) do
    plan = started.plan
    message = plan.persisted.message
  ...
  defp valid_terminal_message?(%Outcome{
    ...
    MessageRecordValidator.validate(message) == :ok and
      message.id === started.plan.persisted.message.id and
      same_message_except_publication?(message, started.plan.persisted.message)
  end
