defmodule ClusterMurmur.Observers.MCPHTTPTransport do
  @moduledoc """
  Executes one fixed observer request through a bounded HTTP connection.

  Each call opens one passive HTTP/1 connection, sends one independently
  revalidated MCP request, incrementally bounds every response body, and closes
  the connection. Redirects, retries, proxies, pooling, and caller-selected HTTP
  ...
  @type result :: MCPHTTPResponse.result() | {:error, :not_sent, :timeout | :unavailable}
  ...
  def execute(%MCPRequest{} = operation, %MCPSettings{} = settings) do
    with {:ok, request} <- MCPHTTPRequest.encode(operation, settings),
      ...
      _invalid -> {:error, :rejected, :invalid_request}
    end
  rescue
    _error -> {:error, :rejected, :invalid_request}
  catch
  ...
  defp send_request(conn, request, uri, scheme, deadline) do
    case safe_configure_send_timeout(conn, scheme, deadline) do
      :ok ->
        dispatch_request(conn, request, uri, scheme, deadline)
  ...
  defp safe_configure_send_timeout(conn, scheme, deadline) do
    ...
    if timeout == 0 do
      ...
      with :ok <- set_socket_options(scheme, socket, options),
           {:ok, actual} <- get_socket_options(scheme, socket, [:buffer]),
           @receive_buffer_bytes <- Keyword.get(actual, :buffer) do
        :ok
      else
  ...
  defp dispatch_request(conn, request, uri, scheme, deadline) do
  ...
  defp decode_response(state) do
    ...
    with {:ok, format} <- response_format(state.status, state.headers) do
      MCPHTTPResponse.decode(%MCPHTTPResponse{
        status: state.status,
        format: format,
        body: body
      })
  ...
  defp response_format(200, headers), do: required_response_format(headers)
  defp response_format(_status, _headers), do: {:ok, :json}

  defp required_response_format(headers) do
  ...
  defp normalize_media_type(value) when is_binary(value) do
    if String.valid?(value),
      do: normalize_valid_media_type(value),
      else: {:error, :invalid_response}
  end

  defp normalize_media_type(_value), do: {:error, :invalid_response}

  defp normalize_valid_media_type(value) do
    media_type =
      value
    ...
    case media_type do
      ...
      "text/event-stream" -> {:ok, :event_stream}
      _unsupported -> {:error, :invalid_response}
    end
  end
