defmodule ClusterMurmur.Runtime.EventRetentionScheduler do
  @moduledoc """
  Runs explicitly configured event-retention cycles without overlap.
  ...
  defmodule Status do
    ...
    @type t :: %__MODULE__{
            ...
            last_result: ClusterMurmur.Runtime.EventRetentionCycle.Result.t() | nil,
            last_error: :retention_failed | :invalid_cycle | nil
          }
  end
  ...
  @doc "Starts one opt-in non-overlapping event-retention scheduler."
  @spec start_link(Options.t()) :: GenServer.on_start()
  def start_link(%Options{} = options) do
    case validate(options) do
      :ok -> GenServer.start_link(__MODULE__, options)
      {:error, :invalid_event_retention_scheduler} = error -> error
    end
  end

  def start_link(_options), do: {:error, :invalid_event_retention_scheduler}

  @doc "Revalidates exact scheduler options without starting a worker."
  @spec validate(term()) :: :ok | {:error, :invalid_event_retention_scheduler}
  def validate(%Options{} = options), do: validate_options(options)
  def validate(_options), do: {:error, :invalid_event_retention_scheduler}

  @doc "Returns aggregate status without configuration or retained marker data."
  ...
  defp next_status({:ok, %EventRetentionCycle.Result{} = result}, status) do
    case EventRetentionCycle.validate_result(result) do
      :ok -> %{status | cycle_count: status.cycle_count + 1, last_result: result, last_error: nil}
      {:error, :invalid_event_retention_cycle_result} -> failed_status(status, :invalid_cycle)
    end
  end

  defp next_status({:error, :event_retention_failed}, status),
    do: failed_status(status, :retention_failed)
  ...
  defp validate_options(options) do
    with true <- exact_options?(options),
      ...
      _failure -> {:error, :invalid_event_retention_scheduler}
    end
  rescue
    _error -> {:error, :invalid_event_retention_scheduler}
  catch
    _kind, _reason -> {:error, :invalid_event_retention_scheduler}
  end
  ...
  defp validate_adapter(adapter, callbacks) do
    if is_atom(adapter) and Code.ensure_loaded?(adapter) and
       ...
       do: :ok,
       else: {:error, :invalid_event_retention_scheduler}
  end
  ...
  defp normalize_configuration(_failure), do: {:error, :invalid_event_retention_scheduler}
  ...
  defp schedule(delay_ms) do
    ...
    Process.send_after(self(), {:event_retention_cycle, timer_token}, delay_ms)
    timer_token
  end
end
