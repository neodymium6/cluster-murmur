defmodule ClusterMurmur.Personas.StarterCooldownRecorder do
  @moduledoc """
  Records the selected persona's cooldown after a proven starter publication.
  ...
  alias ClusterMurmur.Discord.{StarterPublicationExecutor, WebhookSettings}
  ...
  defmodule Recorded do
    ...
    @type t :: %__MODULE__{
            published: ClusterMurmur.Discord.StarterPublicationExecutor.Outcome.t(),
  ...
  @type error ::
          ...
          | :invalid_starter_cooldown
          | :persona_cooldown_conflict
          | :storage_unavailable

  @doc "Records one cooldown from an exact proven starter publication."
  ...
  def record(
    ...
    with :ok <-
           StarterPublicationExecutor.validate(published, configuration, cooldowns, settings),
  ...
  def validate(
    ...
    with true <- exact_recorded?(recorded),
         :ok <-
           StarterPublicationExecutor.validate(
             recorded.published,
             configuration,
             cooldowns,
             settings
      ...
      _failure -> {:error, :invalid_starter_cooldown}
    end
  rescue
    _error -> {:error, :invalid_starter_cooldown}
  catch
    _kind, _reason -> {:error, :invalid_starter_cooldown}
  end
    ...
    case Map.fetch(configuration.personas.personas, persona.id) do
      {:ok, %Persona{} = current} when current === persona ->
        if Validator.validate(current) == :ok,
          do: {:ok, current},
          else: {:error, :invalid_starter_cooldown}
