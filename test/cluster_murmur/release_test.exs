defmodule ClusterMurmur.ReleaseTest do
  ...
  test "applies packaged migrations while stopped and remains idempotent", %{database: database} do
    ...
    assert table_exists?("trigger_executions")
    assert table_exists?("conversations")
    assert table_exists?("messages")
    ...
    assert table_exists?("event_dispatches")
    assert table_exists?("event_dedupe_markers")
