defmodule ClusterMurmur.Discord.ResponderPublicationExecutorTest do
  ...
  @completed_at ~U[2026-08-07 02:00:06Z]
  @normalized_completed_at ~U[2026-08-07 02:00:06.000000Z]
  ...
  test "publishes once and atomically records a correlated success" do
    ...
    assert message === started.plan.delivery.message
  ...
  test "rejects forged capabilities and invalid completion time before publication" do
    ...
    for {candidate, completed_at} <- [
          ...
          {%{started | attempt: %{started.attempt | message_id: 3}}, @completed_at},
          {started, ~U[2026-08-07 02:00:04.000000Z]}
        ] do
      assert {:error, _reason} =
               execute(candidate, configuration, settings, transport, completed_at)
    end

    assert ResponderPublicationExecutor.execute(
             started,
             configuration,
             %{},
             settings,
    ...
    changed_settings = %{
      settings
      | url: "https://discord.com/api/webhooks/1/changed-fake-token"
    }
  ...
  test "rejects forged terminal capabilities" do
    ...
    for forged <- [
          ...
          %{outcome | attempt: %{outcome.attempt | message_id: 3}},
  ...
  defp cooldowns(started), do: started.plan.delivery.plan.input.current_cooldowns
end
