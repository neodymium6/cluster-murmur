# Cluster Murmur v0.2.0-alpha.1

This release promotes the environment-neutral engine into a complete,
fail-closed standalone alpha. It assembles only the fixed reviewed runtime and
transport boundaries; deployment configuration, credentials, endpoints,
storage, and rollout decisions remain operator-owned.

This is still an alpha, not a production-support commitment or permission to
connect the software to sensitive systems.
