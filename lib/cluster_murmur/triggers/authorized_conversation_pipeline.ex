defmodule ClusterMurmur.Triggers.AuthorizedConversationPipeline do
  ...
  @doc "Validates correlated starter and responder adapters without running a pipeline."
  @spec validate_adapters(term()) ::
          :ok | {:error, :invalid_authorized_conversation_pipeline}
  def validate_adapters(%Adapters{} = adapters) do
    with true <- exact_adapters?(adapters),
         ...
         :ok <- AuthorizedStarterPipeline.validate_adapters(adapters.starter),
         :ok <- ResponderTurnCycle.validate_adapters(adapters.responder),
         true <- shared_adapters?(adapters) do
      :ok
    else
      _failure -> {:error, :invalid_authorized_conversation_pipeline}
    end
  ...
  def validate_adapters(_adapters),
    do: {:error, :invalid_authorized_conversation_pipeline}
